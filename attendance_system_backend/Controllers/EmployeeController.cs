...
namespace attendance_system_backend.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        //GET: api/Employee/{employeeId}/{attendanceRecordId}
        [HttpGet("{employeeId}/{attendanceRecordId}")]
        public async Task<IActionResult> GetAttendanceRecordByEmployeeIdAndAttendancerecordId(int employeeId, int attendanceRecordId)
        {
            try
            {
                Response.Headers.Add("Access-Control-Allow-Origin", "*");
                var attendanceRecord = await _employeeService.GetAttendanceRecordByEmployeeIdAndAttendancerecordIdAsync(employeeId, attendanceRecordId);
        ...
        public async Task<IActionResult> AddAttendanceRecod(int employeeId, [FromBody] AttendanceRecordDTO attendanceRecordDto)
        {
            try
            {
                ...
                return Ok(createdAttendanceRecord);
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Internal server error: {ex.Message}");
